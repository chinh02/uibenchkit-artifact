...
For the archived image distribution, verify and load both images:

```bash
sha256sum --check SHA256SUMS
gunzip --stdout uibenchkit-backend-v1.0.0-ase2026.tar.gz | docker load
gunzip --stdout uibenchkit-gui-v1.0.0-ase2026.tar.gz | docker load
```
...
The backend is available at <http://127.0.0.1:5000> and the GUI at
<http://127.0.0.1:3000>. Both ports are bound to loopback only.
...
context excludes these file types. Reviewer keys are passed only at container
runtime. The smoke test sends the bundled screenshot to OpenAI under the terms
of the reviewer's account.
